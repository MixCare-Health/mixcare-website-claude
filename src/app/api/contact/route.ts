...
async function verifyRecaptcha(token: string): Promise<boolean> {
  ...
  if (!secret) {
    // Allow through in environments where the secret hasn't been configured yet,
    // but log clearly so it's visible in server logs.
  ...
  try {
    ...
    // Don't block submission if reCAPTCHA service is unreachable.
    return true;
  }
}
